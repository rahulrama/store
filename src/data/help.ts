...
export const HELP: Record<string, HelpEntry> = {
  ...
  displayBuilt: {
    id: 'displayBuilt',
    term: 'Display built',
    short: 'The promo display is set up as planned.',
    body: 'The promotion’s physical display is built and standing on the shop floor exactly as Head Office intended — the right products in the right place (like an end-of-aisle “end cap” or a feature table), with the promo signage up. If this shows a ✗, customers simply never see the offer.',
    whatToDo: 'Build the display to the campaign brief, then photograph it as proof.',
  },
  ticketed: {
    id: 'ticketed',
    term: 'Ticketed',
    short: 'The promo price tickets and labels are up.',
    body: 'The promotional price tickets and shelf-edge labels are in place, so the deal price actually shows next to the product. Without them a shopper can’t tell the item is on offer — and the price on the shelf may not match the promotion.',
    whatToDo: 'Print and put up the promo tickets; check the shelf price matches the deal.',
  },
  demoRunning: {
    id: 'demoRunning',
    term: 'Demo running',
    short: 'The demo unit is switched on and working.',
    body: 'The demo or “hero” unit is switched on and running the intended content — a console playing the featured game, a TV wall looping the showreel, a smart-home device paired and responding. A live, working demo is what turns a browser into a buyer, so a ✗ here is a missed sale.',
    whatToDo: 'Switch the demo unit on, load the right content, and re-check it through the day.',
  },
  stockAvailable: {
    id: 'stockAvailable',
    term: 'Stock available',
    short: 'Enough sellable stock is out on the shop floor.',
    body: 'There is enough sellable stock out on the shop floor for the promoted line — not sitting unopened in the stockroom. A great display with nothing to sell still loses the sale.',
    whatToDo: 'Bring stock from the stockroom to the floor; if it’s sold out, raise a replenishment or a transfer from another store.',
  },
  soldOutNoneOnOrder: {
    id: 'soldOutNoneOnOrder',
    term: 'Sold out · none on order',
    short: 'Out of stock with no delivery coming.',
    body: 'Lines that are completely sold out on the shop floor and have nothing on order to replace them — so they won’t simply refill themselves. These are the most urgent stock problems: every day they stay empty is a day of lost sales with no fix already on the way.',
    whatToDo: 'Raise a replenishment or a transfer from another store, or offer the customer a fulfilment option from the Assist page.',
  },
  stockRebalance: {
    id: 'stockRebalance',
    term: 'Suggested stock rebalances',
    short: 'Move surplus stock to where it’s sold out.',
    body: 'Where one store has spare stock of a line and a nearby store has sold out, the system suggests moving some across rather than waiting for a supplier delivery. It’s a quick way to turn stock that’s sitting still into sales and plug gaps the same day.',
    whatToDo: 'Review the suggestion and tap “Raise transfer” to request the move.',
  },
  issueClusters: {
    id: 'issueClusters',
    term: 'Issue clusters',
    short: 'The same complaint cropping up repeatedly.',
    body: 'When several customers at the same store raise the same issue, the system groups them into a “cluster” — a signal that it’s a real, recurring problem rather than a one-off. Clusters turn scattered feedback into something you can act on, and each one can be raised as a service-recovery task in a click.',
    whatToDo: 'Open a cluster and raise a task so someone owns fixing the root cause.',
  },
  fulfilment: {
    id: 'fulfilment',
    term: 'Getting it to the customer (fulfilment)',
    short: 'Four ways to save a sale when it’s out of stock here.',
    body: 'When the item a customer wants isn’t available in this store, there are four ways to still make the sale instead of losing it: Reserve & collect (they pick it up from a nearby store), Store transfer (bring it here for them), Same-day courier (send it to their home today from a store that has it), and Ship from store (post it out). The Copilot picks the nearest store with stock and adds the customer’s choice straight to the sale.',
    whatToDo: 'Pick the option that suits the customer and add it to the basket — it’s logged as a recovered sale.',
  },
  domainDepth: {
    id: 'domainDepth',
  ...
  repairDecision: {
    id: 'repairDecision',
    term: 'Repair / replace / write-off',
    short: 'The recommended way to resolve a faulty product.',
    body: 'A recommendation for the best outcome on a faulty item, based on the repair cost, what a new one costs, how much the unit is still worth given its age, and whether it’s under cover. Repair when it’s covered or the fix is cheap relative to the unit’s value; replace when a repair isn’t economical but the item still holds value; write off when an ageing, uncovered item isn’t worth repairing or replacing.',
    whatToDo: 'Go with the recommended action unless the customer’s circumstances call for another — it’s one tap either way.',
  },
  repairCover: {
    id: 'repairCover',
  ...
  promoStockAlert: {
    id: 'promoStockAlert',
    term: 'Promoted stock at risk',
    short: 'A promoted line is running out while the promo is live.',
    body: 'Flags when a product in this campaign is out of stock in one or more stores — so the promotion is driving demand the shelf can’t meet. It prices the gap in lost sales and links straight to Stock, so availability can be fixed before the campaign underperforms for a reason that has nothing to do with the offer itself.',
    whatToDo: 'Open Stock and raise a transfer or replenishment for the short stores.',
  },
  pushToStore: {
    id: 'pushToStore',
    term: 'Push to store',
