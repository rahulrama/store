  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <KpiStat label="Open exceptions" value={openExceptions(regionTasks).length} tone="danger" icon={<TriangleAlert className="size-4" />} />
        <KpiStat label="Overdue" value={overdueTasks(regionTasks, DEMO_NOW).length} tone="warning" icon={<Clock className="size-4" />} />
        <KpiStat label="Completion" value={`${completionRate(regionTasks)}%`} tone="success" icon={<CheckCircle2 className="size-4" />} />
      </div>
